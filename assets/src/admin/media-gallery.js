(function ($) {
	...
	$(document).on('click', '.button--media-uploader', e => {
		e.preventDefault();

		let _this = $(e.target),
			images = _this.attr('data-ids'),
			selection = loadImages(images),
			container = _this.closest('.carousel_slider_images'),
			inputField = container.find('input[type="hidden"]'),
			galleryWrapper = container.find('.carousel_slider_gallery_list');
				...
				insert: {
					...
					click: function () {
						...
						models.each(function (attachment) {
							...
							let thumb = attachment.attributes.sizes.thumbnail || attachment.attributes.sizes.full;
							html += `<li><img src="${thumb.url}" alt="" width="50" height="50" /></li>`;
						});

						this.el.innerHTML = _this.data('progress');

						galleryWrapper.html(html);
						inputField.val(ids.toString());
						_this.attr('data-ids', ids.toString());
